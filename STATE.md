...
## Ontology Redesign (v8 — ACTIVE, Build R1 진입)
- **Pipeline**: `07_ontology-redesign_0410`
- **Phase**: Architect merged ✅ → foundation/ 3축 ✅ → **Build R1 Day 1 대기** (2026-04-12T08:00)
- **설계 SoT**: `20_architect-r1/03_architecture-spec-v3.md`
- **결정**: D1-D20 + Build R1 합의 1/2/3 확정
- **Codex 검증**: CONDITIONAL GO → 조건 반영 완료
- **Build R1 실행 계획**: `30_build-r1/03_impl-plan.md` (5-6일 Day 1-6 + 버퍼 1일)
- **Phase 0 범위**: 신규 10개 테이블 (captures/claims/self_model_traits/self_trait_evidence/self_trait_conflicts/feedback_events/retrieval_logs) + Ollama Qwen2.5-7B claim-extractor + PostUserPrompt hook + policy_rules.json + get_context() 재설계
- **Exit 5가지**: Blind A/B 20문답 ≥70% / shortcut 0건 + 추출률 ≥50% / 50+ traits 8차원 / Single-Rule A/B ("응답 말미 요약 금지") / 잘못된 trait 24h 내 제외
- **Migration Contract**: 10개 규칙 (R1-R10) — 기존 nodes = concepts 리브랜딩, Identity 41 → self_model_traits 시드, Paul 원문 capture 1.5%→60%+ 목표
