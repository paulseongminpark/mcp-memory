# A-16: Phase 0 실행 — 단일 마이그레이션 스크립트 설계

> Round 3 최종 심화 | A-12, A-13, A-11, B-10, B-index 통합
> 목표: `python scripts/migrate_v2_ontology.py` 한 번으로 Phase 0 전체 완료
> 원칙: 각 단계 독립 트랜잭션 + 롤백 가능 + 진행 로그
...
```
Step 1: action_log 테이블 생성 (A-12)
Step 2: meta 테이블 생성 — type_defs, relation_defs, ontology_snapshots (A-13)
Step 3: type_defs 데이터 INSERT — 31 활성 + 19 deprecated (A-13)
Step 4: relation_defs 데이터 INSERT — 49+1 관계 (A-13)
Step 5: 6개 잘못된 관계 교정 + correction_log 기록 (A-11)
Step 6: edges.description JSON 마이그레이션 (B-10)
Step 7: activation_log VIEW 생성 (A-12)
Step 8: ontology_snapshots v2.0-initial 생성 (A-13)
Step 9: nodes 컬럼 추가 — theta_m, activity_history, visit_count (B-index)
```
...
```python
#!/usr/bin/env python3
"""migrate_v2_ontology.py — mcp-memory v2.0 Phase 0 마이그레이션.
...
def step3_insert_type_defs(conn: sqlite3.Connection) -> bool:
    ...
    try:
        ...
        if count > 0:
            _log(3, f"type_defs 이미 {count}행 존재 — 스킵")
            return True
        ...
        conn.commit()
        _log(3, f"type_defs: 31 active + 19 deprecated = 50개 INSERT")
        return True
    except Exception as e:
        conn.rollback()
...
def step4_insert_relation_defs(conn: sqlite3.Connection) -> bool:
    """49+1(governs) = 50개 관계 INSERT."""
    now = datetime.now(timezone.utc).isoformat()
    try:
        count = conn.execute("SELECT COUNT(*) FROM relation_defs").fetchone()[0]
        if count > 0:
            _log(4, f"relation_defs 이미 {count}행 존재 — 스킵")
            return True
...
def step5_correct_invalid_relations(conn: sqlite3.Connection) -> bool:
    ...
    try:
            ...
            for edge in edges:
                conn.execute(
                    """INSERT INTO correction_log
                       ...
                       VALUES (?, 'relation', ?, ?,
                               'A-11: invalid relation corrected (v2 migration)',
                               'migration', ?)""",
        ...
        conn.commit()
        _log(5, f"관계 교정 완료: {total} edges (5개 매핑)")
        return True
    except Exception as e:
        conn.rollback()
...
def step6_migrate_edge_descriptions(conn: sqlite3.Connection) -> bool:
    ...
    try:
        ...
        # 2. 비어있거나 비-JSON → '[]'
        #    "auto: similarity=0.85" 같은 텍스트도 '[]'로 초기화
        #    (원본 텍스트는 이미 edge.reason 필드에 동일 정보 존재)
...
def step8_create_initial_snapshot(conn: sqlite3.Connection) -> bool:
    """A-13: v2.0-initial 스냅샷 생성."""
    now = datetime.now(timezone.utc).isoformat()
    try:
        existing = conn.execute(
            "SELECT id FROM ontology_snapshots WHERE version_tag = 'v2.0-initial'"
        ).fetchone()
        if existing:
            _log(8, "v2.0-initial 스냅샷 이미 존재 — 스킵")
            return True
        ...
        conn.execute(
            ...
            (
                ...
                types_json,
                rels_json,
                "Initial migration: 31 active + 19 deprecated types, "
                "48 active + 2 deprecated relations, "
        ...
        conn.commit()
        _log(8, "ontology_snapshots v2.0-initial 생성")
        return True
    except Exception as e:
        conn.rollback()
...
def migrate():
    """전체 마이그레이션 실행."""
    print(f"\n{'='*60}")
    print(f"mcp-memory v2.0 Phase 0 Migration")
    print(f"DB: {DB_PATH}")
...
### 전체 롤백 (최악의 경우)
```sql
-- 신규 테이블 DROP (기존 데이터 무손실)
DROP VIEW IF EXISTS activation_log;
DROP TABLE IF EXISTS action_log;
DROP TABLE IF EXISTS ontology_snapshots;
DROP TABLE IF EXISTS relation_defs;
DROP TABLE IF EXISTS type_defs;

-- nodes 컬럼은 SQLite에서 DROP COLUMN 불가 → 무시해도 무해
-- edges.description은 이미 텍스트 → 원복 어려움 (백업 필요)
```

### 단계별 롤백
- Step 1-2: `DROP TABLE IF EXISTS ...` 로 즉시 복원
- Step 3-4: `DELETE FROM type_defs; DELETE FROM relation_defs;`
- Step 5: correction_log에 기록된 old_value로 역방향 UPDATE
- Step 6: 백업 없이는 원복 불가 → **실행 전 DB 백업 필수**
- Step 7: `DROP VIEW IF EXISTS activation_log;`
- Step 8: `DELETE FROM ontology_snapshots WHERE version_tag='v2.0-initial';`
- Step 9: SQLite ALTER TABLE DROP COLUMN (3.35.0+) 또는 무시
...
```
[ ] DB 백업: cp data/memory.db data/memory.db.pre-v2
[ ] SQLite 버전 확인: SELECT sqlite_version(); (3.38+ 권장)
[ ] config.py에 ALL_RELATIONS에 'governs' 추가 확인
[ ] 테스트: 빈 DB에서 init_db() + migrate() 실행
[ ] 프로덕션: 백업 확인 후 실행
```
...
| 항목 | 기존 | 변경 후 |
|------|------|---------|
| 테이블 수 | 4 (nodes, edges, sessions, correction_log) | 8 (+action_log, type_defs, relation_defs, ontology_snapshots) |
| VIEW | 0 | 1 (activation_log) |
| nodes 컬럼 | 22개 | 25개 (+theta_m, activity_history, visit_count) |
| edges.description | 텍스트 (auto: similarity=0.85) | JSON 배열 ([{"q":"...", "t":"..."}]) |
| 잘못된 관계 | 5종 20개 edge | 교정 완료 (correction_log 기록) |
| 인덱스 | 8개 | 18개 (+10) |
