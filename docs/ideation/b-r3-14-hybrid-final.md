# B-R3-14: storage/hybrid.py 최종 교체 코드

> 세션 B | Round 3 | 2026-03-05
> 참조: B-10(재공고화), B-11(CTE), B-12(BCM+UCB), A-12(활성화 로깅)
> 대상: `storage/hybrid.py` (현재 119줄 → 목표 ~310줄)
...
| 위치 | 변경 | 내용 |
|---|---|---|
| L1 docstring | CHANGE | Phase 1 버전 언급 |
| L3 | ADD | `import json`, `import math` |
| L6-9 config imports | CHANGE | 6개 신규 상수 추가 |
| L11 graph import | CHANGE | `traverse` 제거 (`build_graph`만 유지) |
| (신규) | ADD | `LAYER_ETA` 상수 dict |
| **L14-44** `_hebbian_update()` | **REPLACE** | `_bcm_update()` 로 완전 교체 |
| (신규) | ADD | `_traverse_sql()` — B-11 보조 CTE |
| (신규) | ADD | `_ucb_traverse()` — B-12 UCB 탐색 |
| (신규) | ADD | `_auto_ucb_c()` — B-12 c값 결정 |
| (신규) | ADD | `_bcm_update()` — B-12+B-10 통합 |
| (신규) | ADD | `_log_recall_activations()` — A-12 |
| L47-52 시그니처 | CHANGE | `excluded_project=""`, `mode="auto"` 추가 |
| **L75-76** | **CHANGE** | `traverse()` → `_ucb_traverse()` (c 적용) |
| L99 필터 블록 | ADD | `excluded_project` 필터 조건 |
| **L116** | **CHANGE** | `_hebbian_update` → `_bcm_update` (scores 포함) |
| (신규) L117 | ADD | `_log_recall_activations()` 호출 |

**핵심**: `_hebbian_update()` 완전 삭제. `_bcm_update()`가 모든 쓰기 작업 통합.
...
```python
# storage/hybrid.py 의존 신규 설정
UCB_C_FOCUS = 0.3          # 집중 모드: 강한 연결 우선
UCB_C_AUTO  = 1.0          # 자동 모드: 균형
UCB_C_DMN   = 2.5          # DMN 모드: 미탐색 우선
BCM_HISTORY_WINDOW = 20    # θ_m 슬라이딩 윈도우 길이
CONTEXT_HISTORY_LIMIT = 5  # edge당 재공고화 맥락 최대 보존 수
# GRAPH_MAX_HOPS = 2 — 이미 존재
# PATCH_SATURATION_THRESHOLD = 0.75 — recall.py 의존 (config에 추가)
```

---

## DB 마이그레이션 SQL (구현 전 필수)
...
-- 2. 기존 3,230 노드 초기화 (SQLite ALTER은 기존 행에 NULL 적용)
...
-- 3. edges.description 마이그레이션 (비-JSON → '[]')
UPDATE edges
SET description = '[]'
...
SELECT COUNT(*) AS total,
       SUM(CASE WHEN theta_m IS NOT NULL THEN 1 ELSE 0 END) AS has_theta
FROM nodes;
```

---

## 최종 코드: storage/hybrid.py 전체
...
def _log_recall_activations(
    ...
    action_log 미구현(Phase 1) 시 ImportError → graceful skip.
    Phase 2에서 storage/action_log.py 구현 후 자동 활성화.
    """
    try:
        from storage import action_log  # Phase 2에서 구현
        ...
        pass  # action_log 미구현 — Phase 2 활성화
    except Exception:
...
def hybrid_search(
    ...
    # 4. UCB 그래프 탐색 (B-12)
    #    all_edges: _bcm_update 의존 + build_graph 의존 → Phase 2에서 제거
    ...
    # 8. 활성화 이벤트 로깅 (A-12, Phase 2에서 action_log 구현 시 활성화)
...
### 1. `_hebbian_update()` 완전 제거
B-10과 B-12를 합치면 `_hebbian_update()`는 `_bcm_update()`의 완전한 상위셋.
BCM이 `frequency+1` (헤비안)을 포함하므로 중복 없음.

**주의**: `delta_w * 10` 스케일링 — frequency가 0~100 범위이므로 delta_w(0~0.02)를
10배 스케일링해 반영. B-1 설계 기준. 필요 시 조정.

### 2. `_traverse_sql()` 유지 이유
Phase 1에서는 호출되지 않지만:
- Phase 2 전환 시 즉시 `_ucb_traverse` 대체 가능
- `build_graph` 제거 로드맵의 준비
- 함수 존재 자체가 문서화 역할

### 3. `_log_recall_activations()` graceful skip
`storage/action_log.py` 미구현 → `ImportError` → `pass`.
Phase 2에서 action_log 모듈 생성 시 코드 변경 없이 자동 활성화.

### 4. `visit_count` + `last_activated` 통합
기존 `_hebbian_update()`에는 `nodes.last_activated` 업데이트가 없었음.
`_bcm_update()`에서 visit_count와 함께 `last_activated` 동시 갱신.
→ D-5 temporal_search가 의존하는 `last_activated` 최신 상태 보장.
...
| 전체 줄 수 | 119줄 | ~310줄 |
| 함수 수 | 2 | 7 |
| DB 쓰기 (recall당) | N UPDATEs | 3N + K UPDATEs |
| DB 읽기 (BCM) | 0 | N SELECTs (activated edge 수) |
| NetworkX 의존 | graph 빌드 | graph 빌드 (UCB) — Phase 2에서 제거 |
